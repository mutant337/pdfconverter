...
from PyPDF2 import PdfFileMerger, PdfFileWriter, PdfFileReader, PdfReader
...
for row in sheet.iter_rows(min_row=2, values_only=True):
    source_path, priority, destination, numeration, end = row[0], row[1], row[2], row[3], row[4]
    ...
    if source_path:  # Если ячейка непустая
        ...
        if numeration:
            numeration_list.append(source_path)
        ...
        if end:
            ...
            for i in sorted_dict:
                ...
                # Якщо старий шлях з нумерацією, замінюємо на новий
                if file_path in numeration_list:
                    index = numeration_list.index(file_path)
                    numeration_list[index] = converted_file_path
                ...
                try:
                    ...
                    # adding blank page
                    a = open(converted_file_path, 'rb')
                    pdf = PdfFileReader(a, strict=False)
                    ...
                    if numPages % 2 != 0:
                        ...
                        shutil.copyfile('Amended.pdf', converted_file_path)
                        os.remove("Amended.pdf")
                    else:
                        a.close()

                    # adding paginatin
                    if converted_file_path in numeration_list:
                        add_text_to_pdf(converted_file_path, converted_file_path+'1.pdf')
                        os.remove(converted_file_path)
                        os.rename(converted_file_path+'1.pdf', converted_file_path)
            ...
            flag, data_dict, result, n = True, {}, '', 0
